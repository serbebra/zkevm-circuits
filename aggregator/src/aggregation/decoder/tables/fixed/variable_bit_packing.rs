...
impl FixedLookupValues for RomVariableBitPacking {
    fn values() -> Vec<[Value<Fr>; 7]> {
        ...
        //
        // witness_debug: TODO(ray) switch this again when its time to test RomVariableBitPacking.
        // For now I am reducing the number of rows it takes, so that we can continue using k=11.
        // let rows = (1..=513)
        let rows = (1..=1)
            .flat_map(|range| {
