...
use crate::{
    ...
    // AggregationConfig, RlcConfig,
    BITS,
    CHUNK_DATA_HASH_INDEX,
    LIMBS,
    POST_STATE_ROOT_INDEX,
    PREV_STATE_ROOT_INDEX,
    WITHDRAW_ROOT_INDEX,
};
...
pub(crate) fn extract_accumulators_and_proof(
    ...
    let accumulators = snarks
        ...
        .flat_map(|snark| {
            ...
            let proof = PlonkSuccinctVerifier::<KzgAs<Bn256, Bdfg21>>::read_proof(
                &svk,
                &snark.protocol,
                &snark.instances,
                &mut transcript_read,
...
// padded // 4. chunks are continuous: they are linked via the state roots
...
//     // 6. chunk[i]'s chunk_pi_hash_rlc_cells == chunk[i-1].chunk_pi_hash_rlc_cells when chunk[i]
// is     // padded
//     // 7. the hash input length are correct
//     // - first MAX_AGG_SNARKS + 1 hashes all have 136 bytes input
//     // - batch's data_hash length is 32 * number_of_valid_snarks
...
//                     }
//                     if offset % keccak_f_rows == 0 && offset / keccak_f_rows <=
// max_keccak_updates {                         // first column is is_final
...
//                             &chunk_pi_hash_preimages[MAX_AGG_SNARKS - 1][i +
// POST_STATE_ROOT_INDEX]                                 .value(),
//                         )
//                         .as_str(),
//                     )?;
//                     region.constrain_equal(
...
//                     constrain_flags(rlc_config, &mut region, &chunk_is_valid_cells, &mut
// offset)?;
...
//                 //
//                 // else if the
//                 // num_of_valid_snarks <= 12, which needs 3 keccak-f rounds. Therefore the
// batch's                 // data hash (input, len, data_rlc, output_rlc) are in the 3rd 300 keccak
// rows;                 //
//                 // else if the
//                 // num_of_valid_snarks <= 16, which needs 4 keccak-f rounds. Therefore the
// batch's                 // data hash (input, len, data_rlc, output_rlc) are in the 4th 300 keccak
// rows;                 //
...
// j].value(),                                 &chunk_pi_hash_preimages[i][POST_STATE_ROOT_INDEX +
// j].value(),                                 &chunk_is_valid_cells[i + 1].value(),
//                             )
//                             .as_str(),
//                         )?;
//                         rlc_config.conditional_enforce_equal(
...
//                 // 6. chunk[i]'s chunk_pi_hash_rlc_cells == chunk[i-1].chunk_pi_hash_rlc_cells
// when                 // chunk[i] is padded
//                 let chunks_are_padding = chunk_is_valid_cells
//                     .iter()
//                     .map(|chunk_is_valid| rlc_config.not(&mut region, chunk_is_valid, &mut
// offset))                     .collect::<Result<Vec<_>, halo2_proofs::plonk::Error>>()?;
...
//                 // 2*(MAX_AGG_SNARKS) + 2    | 1     |   chunk[MAX_AGG_SNARKS].pi_hash use 2
// rounds                 // 2*(MAX_AGG_SNARKS) + 3    | a     | MAX_AGG_SNARKS+2's keccak
...
//                 region
//                     .constrain_equal(left.cell(),
// rlc_config.one_cell(left.cell().region_index))?;
