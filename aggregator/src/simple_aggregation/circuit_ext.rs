...
use super::SimpleAggregationCircuit;

impl CircuitExt<Fr> for SimpleAggregationCircuit {
    fn num_instance(&self) -> Vec<usize> {
        // re-expose inner public input; excluding ACC_LEN for each snark
        let snark_pi_len: usize = self
            .snarks
            .iter()
            .flat_map(|snark| snark.instances.iter().map(|x| x.len() - ACC_LEN))
            .sum();

        vec![snark_pi_len + ACC_LEN]
    }
