...
/// generated.
pub static GETH0_URL: Lazy<String> = Lazy::new(|| match env::var("GETH0_URL") {
    Ok(val) => val,
    Err(VarError::NotPresent) => GETH0_URL_DEFAULT.to_string(),
    Err(e) => panic!("Error in GETH0_URL env var: {e:?}"),
});
/// ..
pub static START_BLOCK: Lazy<usize> = Lazy::new(|| match env::var("START_BLOCK") {
    Ok(val) => str::parse::<usize>(&val).unwrap(),
    Err(VarError::NotPresent) => 16140010,
    Err(e) => panic!("Error in START_BLOCK env var: {e:?}"),
});
/// ..
pub static END_BLOCK: Lazy<usize> = Lazy::new(|| match env::var("END_BLOCK") {
    Ok(val) => str::parse::<usize>(&val).unwrap(),
    Err(VarError::NotPresent) => 16140010,
    Err(e) => panic!("Error in END_BLOCK env var: {e:?}"),
});
/// ..
pub static TX_ID: Lazy<String> = Lazy::new(|| match env::var("TX_ID") {
    Ok(val) => val,
    Err(VarError::NotPresent) => "".to_string(),
    Err(e) => panic!("Error in TX_ID env var: {e:?}"),
});
/// ..
pub static CIRCUIT: Lazy<String> = Lazy::new(|| match env::var("CIRCUIT") {
    Ok(val) => val,
