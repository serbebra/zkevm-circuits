...
#[derive(Clone)]
pub(crate) enum MayStaticString {
    Static(&'static str),
    Owned(Arc<String>),
}

impl From<&'static str> for MayStaticString {
    fn from(s: &'static str) -> Self {
        Self::Static(s)
    }
}

impl From<String> for MayStaticString {
    fn from(s: String) -> Self {
        Arc::new(s).into()
    }
}

impl From<Arc<String>> for MayStaticString {
    fn from(s: Arc<String>) -> Self {
        Self::Owned(s)
    }
}

impl std::fmt::Debug for MayStaticString {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MayStaticString::Static(s) => f.write_str(s),
            MayStaticString::Owned(s) => f.write_str(s),
        }
    }
}

impl std::fmt::Display for MayStaticString {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MayStaticString::Static(s) => f.write_str(s),
            MayStaticString::Owned(s) => f.write_str(s),
...
impl std::ops::Deref for MayStaticString {
    type Target = str;

    fn deref(&self) -> &Self::Target {
        match self {
            MayStaticString::Static(s) => s,
            MayStaticString::Owned(s) => s,
        }
    }
}
