...
use crate::evm_circuit::util::{from_bytes, rlc, CachedRegion, Cell};
...
impl<F: FieldExt> Word32Cell<F> {
    pub(crate) fn new(limbs: [Cell<F>; 32], randomness: Expression<F>) -> Self {
        Self {
            expression: rlc::expr(&limbs.clone().map(|cell| cell.expr()), randomness),
            limbs,
        }
    }
...
impl<F: FieldExt> Expr<F> for Word32Cell<F> {
