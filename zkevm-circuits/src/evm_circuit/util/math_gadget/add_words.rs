...
pub(crate) struct AddWordsGadget<F, const N_ADDENDS: usize, const CHECK_OVERFLOW: bool> {
    addends: [util::Word32Cell<F>; N_ADDENDS],
    ...
    AddWordsGadget<F, N_ADDENDS, CHECK_OVERFLOW>
{
    pub(crate) fn construct(
        ...
        let addends_lo = &addends
            .iter()
            .map(|addend| from_bytes::expr(&addend.limbs[..16]))
            .collect::<Vec<_>>();
        let addends_hi = &addends
            .iter()
            .map(|addend| from_bytes::expr(&addend.limbs[16..]))
            .collect::<Vec<_>>();
        let sum_lo = from_bytes::expr(&sum.limbs[..16]);
        let sum_hi = from_bytes::expr(&sum.limbs[16..]);
    ...
    pub(crate) fn addends(&self) -> &[util::Word32Cell<F>] {
        &self.addends
    }

    pub(crate) fn sum(&self) -> &util::Word32Cell<F> {
        &self.sum
    }
