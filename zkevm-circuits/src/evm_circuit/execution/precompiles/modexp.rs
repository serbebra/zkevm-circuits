...
use crate::{
    evm_circuit::{
        ...
        step::ExecutionState,
        util::{constraint_builder::EVMConstraintBuilder, CachedRegion, Cell,Word},
    },
    table::{CallContextFieldTag, CopyTable},
...
pub struct ModExpGadget<F> {
    ...
    // use 6 cells for the lengths specify and its bit represent (32 at most)
    base_len: [Cell<F>;6],
    modulus_len: [Cell<F>;6],
    exp_len: [Cell<F>;6],
    randomness: [Cell<F>;6], //the exponents of randomness: 2,4,8,16,32,64
    base: Word<F>,
    modulus: Word<F>,
    exp: Word<F>,
    result: Word<F>,
