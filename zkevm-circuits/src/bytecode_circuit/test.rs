...
fn set_unit_test_env_var(value: &str) {
    std::env::set_var("UNIT_TEST_ASSIGNMENT_TYPE", value);
    let assign_var = std::env::var("UNIT_TEST_ASSIGNMENT_TYPE")
        .ok()
        .unwrap_or_default();
    log::info!("UNIT_TEST_ASSIGNMENT_TYPE: {}", assign_var);
}
...
fn bytecode_circuit_parallel_assignment() {
    ...
    set_unit_test_env_var("parallel");
    create_proof::<
