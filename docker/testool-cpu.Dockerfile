...
RUN mkdir configs
RUN curl -o ./configs/layer1.config https://circuit-release.s3.us-west-2.amazonaws.com/release-v0.9.1/layer1.config
RUN curl -o ./configs/layer2.config https://circuit-release.s3.us-west-2.amazonaws.com/release-v0.9.1/layer2.config
...
COPY --from=builder /src/configs/ /opt/configs/
